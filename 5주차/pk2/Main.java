...
import pk2.MemoryMemberRepository;
import pk2.MockMemberRepository;
import role.Lion;
import role.Role;
import role.Staff;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("1. MemoryMemberRepository (실제저장)");
        System.out.println("2. MockMemberRepository (더미데이터)");
        System.out.print("선택 : ");
            int se = sc.nextInt();
            if(se==1){
                MemberRepository realRepo = new MemoryMemberRepository();
                memberService = new pk2.MemberService(realRepo);

            }
            else{
                MemberRepository mockRepo = new MockMemberRepository();
                memberService = new pk2.MemberService(mockRepo);
            }
        while (true) {
            System.out.println("\n======== 멤버 시스템 관리 =======");
